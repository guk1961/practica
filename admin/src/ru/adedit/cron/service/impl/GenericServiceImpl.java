...
public abstract class GenericServiceImpl<T extends DomainObject> implements IGenericService<T> {
	...
	private static final String QUERY_SELECT_BY_ENTITY = "SELECT x FROM %s x WHERE x =?1"; //$NON-NLS-1$
	private static final String QUERY_SELECT_BY_ID_LIST = "SELECT x FROM %s x WHERE x.id IN (?1)"; //$NON-NLS-1$
	private static final String QUERY_SELECT_ALL = "SELECT x FROM %s x ORDER BY x.name ASC"; //$NON-NLS-1$
	private static final String QUERY_COUNT_ALL = "SELECT COUNT(x) FROM %s x"; //$NON-NLS-1$
	private static final String QUERY_SELECT_BY_NAME = "SELECT x FROM %s x WHERE x.name LIKE ?1 ORDER BY x.rank, x.name"; //$NON-NLS-1$

//	@PersistenceUnit(unitName="hc")
//	@PersistenceContext
	...
	public void init(){
//		entityManager = Adedit.getInstance().getEntityManager();	
...
//		this.entityManager = jpaTemplate.getEntityManager();
	}
...
//	@Override
	...
	public T save(T entity) throws IllegalArgumentException, GeneralServiceException {
		...
		log.debug("Попытка сохранить сущность " + entity);
		T savedEntity = null;
	 ...
	 */
	@Transactional(propagation = Propagation.REQUIRED, readOnly = false)
	public void delEntity(Integer id) throws IllegalArgumentException, GeneralServiceException {
		if (id == null) {
	 ...
	 */
	@Transactional(propagation = Propagation.REQUIRED, readOnly = false)
	public void delAllEntities(Collection<Integer> ids) throws IllegalArgumentException, 
	...
	@Transactional(propagation = Propagation.SUPPORTS, readOnly = true)
	public List<T> getEntitesByName(String name) {
		try {
			// Getting all entities
			List<T> list = executeQuery(String.format(QUERY_SELECT_BY_NAME, persistentClass
					.getSimpleName()), false, false, name);
			if (list != null)
				return list;
			else
				return new ArrayList<T>();
	...
	@Transactional(propagation = Propagation.SUPPORTS, readOnly = true)
	public T getEntity(T ent) throws InstantiationException, IllegalAccessException {
		try {
			// Getting all entities
			T entity = executeQuery(String.format(QUERY_SELECT_BY_ENTITY, persistentClass
					.getSimpleName()), false, true,ent);
			if (entity != null)
				return entity;
			else
	 ...
	 */
	@Transactional(propagation = Propagation.SUPPORTS, readOnly = true)
	public List<T> getEntitiesByIds(List<Integer> ids) throws IllegalArgumentException {
